  ...
  if (print_additions_flag)
  {
    if(!parts.isArray)
    {
      ...
      if(parts.type == T_INT)
        ...
        cout << "BOOLEAN";
      else if (parts.type == T_PROG)
        cout << "PROGRAM";
      else if (parts.type == T_PROC)
        cout << "PROCEDURE";
      cout << endl;
    }
    else
...
void Parser::printRule(const string lhs, const string rhs)
{
  if (print_productions_flag)
    cout << lhs << " -> " << rhs << " \tArray is: " << array_temp.isArray << endl; 
  return;
}
...
TYPE_INFO Parser::expr(Lexer &lex, vector<string> &currentToken) 
{
  ...
  cout << "EXEC<-simpleexpr TMP1: " << tmp_info1.type << endl;
  cout << "EXEC<-opExpr TMP2: " << tmp_info2.type << endl;
...
{
  TYPE_INFO tmp_info;
  cout << "IDX VAR GOT: " << currentToken[0] << " :: " << currentToken[1] << endl;
  if (currentToken[0] == "T_LBRACK")
  {
    printRule("N_IDXVAR", "T_LBRACK N_EXPR T_RBRACK");
    currentToken = lex.getToken();
    tmp_info = expr(lex, currentToken);
    cout << "IDX VAR GOT: " << tmp_info.type << endl;
